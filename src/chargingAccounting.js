...
function computeQuarterSnapshot({ year, quarter, sessions, dailyTotals, rates, settings }) {
  ...
  const entries = quarterSessions.map((session) => {
    ...
    return {
      ...
      soc_end_pct: Number.isFinite(session.soc_end_pct) ? session.soc_end_pct : null,
      timezone: session.timezone || '',
      charger: session.charger || '',
      vehicle: session.vehicle || '',
      manual_override: Boolean(session.manual_override),
      manual_override_note: session.manual_override_note || '',
      manual_override_at: session.manual_override_at || null,
      manually_updated_at: session.manually_updated_at || null,
